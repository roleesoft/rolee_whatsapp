...
exports.rolee_whatsappFooter = '\u2B50 Please star Rolee_WhatsApp on GitHub! https://github.com/roleesoft/rolee_whatsapp';
function getAuthStateCacheFolderLocation() {
    if (process.env.ROLEE_WHATSAPP_CACHE_FOLDER) {
        return process.env.ROLEE_WHATSAPP_CACHE_FOLDER;
    }
    else {
        const homedir = os.homedir();
        if (process.platform === 'win32') {
            return path_1.default.join(homedir, 'AppData', 'Local', 'rolee_whatsapp', 'Data');
        }
        else {
            return path_1.default.join(homedir, '.local', 'share', 'rolee_whatsapp');
        }
    }
}
...
function initAuthStateCacheFolder() {
    ...
    if (!fs.existsSync(folderLocation)) {
        ...
        signale_1.default.log(`Created rolee_whatsapp cache folder: ${folderLocation}`);
    }
