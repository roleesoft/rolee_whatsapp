...
export const rolee_whatsappFooter = '\u2B50 Please star Rolee_WhatsApp on GitHub! https://github.com/roleesoft/rolee_whatsapp';
...
export function getAuthStateCacheFolderLocation() {
    if (process.env.ROLEE_WHATSAPP_CACHE_FOLDER) {
        ...
        if (process.platform === 'win32') {
            return path.join(homedir, 'AppData', 'Local', 'rolee_whatsapp', 'Data');
        } else {
            return path.join(homedir, '.local', 'share', 'rolee_whatsapp');
        }
    }
}
...
function initAuthStateCacheFolder() {
    ...
    if (!fs.existsSync(folderLocation)) {
        ...
        signale.log(`Created rolee_whatsapp cache folder: ${folderLocation}`);
    }
